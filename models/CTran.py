...
class CTranModel(nn.Module):
    def __init__(self,num_labels,use_lmt=True,pos_emb=False,layers=3,heads=4,dropout=0.1,no_x_features=False):
        ...
        # model = gensim.models.Word2Vec(corpus_file='/home/whoami/Documents/Codes/Ctran/TrainEmbeddings.txt', vector_size=2048, window=2, min_count=1)
        ...
        # Label Embeddings
        self.label_input = torch.Tensor(np.arange(num_labels)).view(1,-1).long() # label_input.size = torch.Size([1, 17])
    ...
    def forward(self,images,mask):
        ...
        # const_label_input.size() = torch.Size([batch_size, 17])
    
        init_label_embeddings = self.label_lt(const_label_input) # init_label_embeddings.size() = torch.Size([batch_size, 17, 2048])
        # init_label_embeddings = self.label_lt.repeat(images.size(0),1, 1)
        
        features = self.backbone(images) # torch.Size([batch_size, 1024, 14, 14])
        ...
        features = features.view(features.size(0),features.size(1),-1).permute(0,2,1) # features.size() = torch.Size([batch_size, 196, 2048])

        if self.use_lmt:
            # Convert mask values to positive integers for nn.Embedding
            label_feat_vec = custom_replace(mask,0,1,2).long()
            # label_feat_vec.size() = torch.Size([batch_size, 17]) 

            # Get state embeddings
            state_embeddings = self.known_label_lt(label_feat_vec)
            # state_embeddings.size() = torch.Size([batch_size, 17, 2048])

            # Add state embeddings to label embeddings
            init_label_embeddings += state_embeddings
            # init_label_embeddings.size() = torch.Size([batch_size, 17, 2048])
        ...
        label_embeddings = embeddings[:,-init_label_embeddings.size(1):,:]  # label_embeddings.size = torch.Size([batch_size, 17, 2048])
        
        output = self.output_linear(label_embeddings)  # output.size() = torch.Size([batch_size, 17, 17])

        diag_mask = torch.eye(output.size(1)).unsqueeze(0).repeat(output.size(0),1,1).cuda()
        output = (output*diag_mask).sum(-1) # output.size() = torch.Size([batch_size, 17])
        ...
        # output = 0.8*output + 0.2*gcn_features
