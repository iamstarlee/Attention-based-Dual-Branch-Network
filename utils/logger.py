...
class LossLogger:
    ...
    def log_losses(self,file_name,epoch,loss,metrics,loss_unk, lr, start):
        ...
        if file_name == 'train.log':
            writer.add_scalar('Training mAP', metrics['mAP'], epoch)
            writer.add_scalar('Training loss', metrics['loss'], epoch)
            writer.add_scalar('Training unknown loss', metrics['lossu'], epoch)
            writer.add_scalar('Learning Rate', lr, epoch)
            writer.add_scalar('Training time', time.time()-start, epoch)
        elif file_name == 'valid.log':
            writer.add_scalar('Validating mAP', metrics['mAP'], epoch)
            writer.add_scalar('Validating loss', metrics['lossu'], epoch)
            writer.add_scalar('Validating time', time.time()-start, epoch)
...
class Logger:
    ...
    def evaluate(self,train_metrics,valid_metrics,test_metrics,epoch,num_params,model,valid_loss,test_loss,all_preds,all_targs,all_ids,args):
        ...
        if args.dataset == 'cub':
            ...
            print('\n')
            print('**********************************')
            print('best mAP:  {:0.1f}'.format(self.best_test['mAP']))
            print('best CF1:  {:0.1f}'.format(self.best_test['CF1']))
            print('best OF1:  {:0.1f}'.format(self.best_test['OF1']))
            print('**********************************')
